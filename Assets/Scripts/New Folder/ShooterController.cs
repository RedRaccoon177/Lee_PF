...
public class ShooterController : MonoBehaviour
{
    ...
    void Move()
    {
        if (_moveInput.magnitude == 0) return; // 입력값이 없으면 이동하지 않음

        float speed = _isRunning ? RunSpeed : WalkSpeed; // 달리기 상태에 따라 이동 속도 결정
    ...
    void UpdateAnimation()
    {
        ...
        float speedFactor = _isRunning ? 1.0f : 0.5f; // Shift를 누르면 1.0 (달리기), 걷기는 0.5
        float targetSpeed = Mathf.Clamp(_moveInput.magnitude * speedFactor, 0.0f, 1.0f); // 0~1 사이로 제한
        ...
        if (_moveInput.magnitude > 0) // 이동 중일 때만 방향 계산
        {
    ...
    {
        ExecuteCommand(new ShooterShootCommand(this));
    }

    public void ExecuteCommand(ShooterICommand command)
    {
        // 명령 객체 실행
        command.Execute();
    }
